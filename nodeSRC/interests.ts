...
export class SuperInterest extends Interest {
    ...
    //builds a new Interests from a json of the object
    public static fromjson(j)
    {
        let i:SuperInterest = new SuperInterest(j["_name"],j["state"]);
        for (let index = 0; index < j['subInterests'].length; index++) {
            const element = j['subInterests'][index];
            i.addSubInterest(new Interest(element['_name'],element['state']));
        }
        return i;
    }
...
export class Interests 
{
    ...
    //builds a new Interests from data generated by toJSON(and acceptable modification thereof)
    public static fromJSON(j) : Interests{
        let i:Interests = new Interests();
        for (let index = 0; index < j['interests'].length; index++) {
            const element = j['interests'][index];
            i.setInterest(SuperInterest.fromjson(element[1]));
        }
        return i;
    }
}
